...
├── config/
│   ├── config.php            # Charge database.php (PDO)
...
```sql
-- Copier le contenu de config/database.sql dans PhpMyAdmin
```

Ou via ligne de commande:

```bash
mysql -u root < C:\xampp\htdocs\recette\config\database.sql
```
...
→ Vérifier que `config/database.sql` a été exécuté
